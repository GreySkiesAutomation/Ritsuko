...
namespace Runtime.Avatar
{
    public class SpeakAndEmoteController : PearlBehaviour
    {
        ...
        private void SendAndGetEmotion(bool promptAfterwards = false)
        {
            ...
            {
                LogError("No OpenRouterChatClient assigned.");
                return;
            }
        ...
        private async Task SendRoutine(bool promptAfterwards = false)
        {
            ...
            try
            {
                var result = await GlobalManager.I.LlmClient.SendPromptAsync($"Given this dialogue: {_thingToSpeak}\n" +
                                                                             ...
                                                                             "4: Annoyed\n" +
                                                                             "5: Surprised (negative)\n", GlobalManager.I.Configuration.LlmModelFast);
                _reply = result;
                ...
                if (promptAfterwards)
                {
                    //if GlobalManager.I.TtsPlayer.IsSpeaking, wait until done, then prompt for next thing to say after a short delay   
